...
function _CheckTpm {
    ...
    switch ($CheckTpm) {
        ...
        { $CheckTpm.TpmPresent -eq $True -and $CheckTpm.TpmEnabled -eq $True } { return $True }
        Default { return $False }
    }
...
if (_CheckSecureBoot -eq $True) {
    ...
    Write-Host "FAIL " -ForegroundColor Red -NoNewline
    Write-Output "Votre machine n'est pas compatible Secureboot ou verifiez qu'il soit activer sur votre machine"
}
